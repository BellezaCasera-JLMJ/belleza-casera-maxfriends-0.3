document.addEventListener('DOMContentLoaded', function() {
  ...
  },
  {
    ...
    categoria: "manos",
    imagen: "https://citymagazine.b-cdn.net/wp-content/uploads/2025/07/hero-cream-for-dry-soles-2025--1400x933.webp",
    ingredientes: [
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  entrarBtn.addEventListener('click', function() {
    ...
    // Mostrar el resto de la app
    document.querySelector('header').style.display = '';
    document.querySelector('main').style.display = '';

    // Ocultar el mensaje después de 2s
    setTimeout(() => {
      bienvenidaMsg.classList.remove('mostrar');
    }, 2000);
  });
});
// Paleta de colores suaves y llamativos
const coloresSuaves = [
  "#ffe5ec", // rosa suave
  "#e0f7fa", // azul celeste
  "#fcf6bd", // amarillo pastel
  "#d0f4de", // verde agua
  "#ffd6e0", // rosado claro
  "#fde4cf", // durazno
  "#c7d8f4", // azul lavanda
  "#fff1f0", // coral pastel
  "#e7c6ff", // lila pastel
  "#f9f6ff", // blanco violeta
  "#d8e2dc", // gris rosado
  "#fceabb", // amarillo crema
  "#b5ead7", // verde menta
  "#fcd5ce", // naranja claro
  "#b5ead7", // verde pastel
  "#f9c6c9", // rosado sandía
];

// Asigna un color único a cada receta card
function aplicarColoresRecetas() {
    document.querySelectorAll('.receta-card').forEach((card, idx) => {
        const color = coloresSuaves[idx % coloresSuaves.length];
        card.style.background = color;
        // Para que el borde combine, opcional:
        card.style.borderColor = color;
    });
}
document.addEventListener('DOMContentLoaded', aplicarColoresRecetas);
// Llama también después de renderizar recetas dinámicamente:
if (typeof renderRecetas === "function") {
  const orig = renderRecetas;
  window.renderRecetas = function(...args){
    orig.apply(this, args);
    aplicarColoresRecetas();
  }
}
// Animación al marcar/desmarcar favorito
function toggleFavorito(btn) {
  btn.classList.toggle('active');
  // Puedes aquí agregar lógica para guardar favorito real
}
// Abrir/cerrar modal con transición
...
function showToast(mensaje = "¡Acción realizada!") {
  ...
  }, 2300); // Duración visible (ms)
}

// Ejemplo: úsalo al agregar a favoritos
...
// Inicializa según preferencia guardada o sistema
const prefersDark = window.matchMedia('(prefers-color-scheme: dark)').matches;
const darkMode = localStorage.getItem('darkMode');
setTheme(darkMode === '1' || (darkMode === null && prefersDark));
